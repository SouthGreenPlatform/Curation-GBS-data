...
#'
#' @examples
multiple_check_all<-function(scaff_list,genot_data,id,type,code="abh-",mc=marker_column,sc=scaffold_column){
  out<-NULL
  for(scaff in scaff_list)
    out<-rbind(out, multiple_check(scaff,genot,id,"BC1",code=code,mc,sc) )
  out
}
