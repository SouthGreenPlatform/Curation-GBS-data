...
#p.value cut-off Suggested something like 1/(2* number of rows). Negative values cancel the filter
P.value.min <- -1
# Cross type: (only BC1 is implemented currently)
type <- "BC1"
# Name the parents. Two values are required (In BC1, the heterozygous parent comes first).
...
where_out_2 <-"data/set1/step2"
  
file.value.2 <- "genot_first_round_2.txt"
file.ident.2 <- "markid_first_round_2.txt"
...
where_out_3 <- "data/set1/step3"
# name of coded genotype file
file.value.3 <- "genot_first_round_3.txt"
#name of updated marker info file 
file.ident.3 <- "markid_first_round_3.txt"
...
comment5 <- ""
# place of final files
where_out_5 <- "data/set1/step5"
#Number of individuals used for plotting
genot_file_5<-"data_joinmap.txt"
infoloc_5<-"info_loc.txt"
